...
TR::IlValue *OMR::IlBuilder::Copy(TR::IlValue *value)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is Copy value (%d) dataType (%d) at cpIndex (%d)\n", this, newVal->getID(),
        value->getID(), dt.getDataType(), newVal->getID(), newSymRef->getCPIndex());
...
TR::BytecodeBuilder *OMR::IlBuilder::OrphanBytecodeBuilder(int32_t bcIndex, const char *name)
{
...
TR::IlValue *OMR::IlBuilder::StructFieldInstanceAddress(const char *structName, const char *fieldName, TR::IlValue *obj)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d was computed for StructFieldInstanceAddress %s.%s with base %d\n", this,
        typedAddr->getID(), structName, fieldName, obj->getID());
    return typedAddr;
}
...
TR::IlValue *OMR::IlBuilder::UnionFieldInstanceAddress(const char *unionName, const char *fieldName, TR::IlValue *obj)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d was computed for UnionFieldInstanceAddress %s.%s with base %d\n", this,
        typedAddr->getID(), unionName, fieldName, obj->getID());
    return typedAddr;
}
...
void OMR::IlBuilder::Return(TR::IlValue *value)
{
    ...
    if (value->getDataType() == TR::Int8 || value->getDataType() == TR::Int16
        || (Word == Int64 && value->getDataType() == TR::Int32)) {
...
TR::IlValue *OMR::IlBuilder::genCall(const char *name, TR::SymbolReference *methodSymRef, int32_t numArgs,
    TR::IlValue **argValues, bool isDirectCall /* true by default*/)
{
...
TR::IlValue *OMR::IlBuilder::AtomicAdd(TR::IlValue *baseAddress, TR::IlValue *value)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is AtomicAdd(%d, %d)\n", this, returnValue->getID(), baseAddress->getID(),
        value->getID());
    return returnValue;
}
    ...
    TR::IlBuilder **transactionBuilder)
{
    ...
    *transactionBuilder = createBuilderIfNeeded(*transactionBuilder);
    ...
    TraceIL("IlBuilder[ %p ]::Transaction starting persistentFail %p transientFail %p transaction %p\n", this,
        *persistentFailureBuilder, *transientFailureBuilder, *transactionBuilder);
...
void OMR::IlBuilder::traceSwitch(const char *name, TR::IlValue *selectorValue, TR::IlBuilder *defaultBuilder,
...
TR::IlValue *OMR::IlBuilder::Select(TR::IlValue *condition, TR::IlValue *trueValue, TR::IlValue *falseValue)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is Select %d T (%d) F (%d)\n", this, result->getID(), condition->getID(),
        trueValue->getID(), falseValue->getID());
    return result;
}
