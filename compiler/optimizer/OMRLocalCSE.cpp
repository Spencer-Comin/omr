...
bool OMR::LocalCSE::shouldCopyPropagateNode(TR::Node *parent, TR::Node *node, int32_t childNum, TR::Node *storeNode)
{
    ...
    if (_numCopyPropagations >= MAX_COPY_PROP) {
        logprintf(trace(), comp()->log(), "z^z : _numCopyPropagations %d >= max %d\n", _numCopyPropagations,
            MAX_COPY_PROP);
        return false;
    }
...
TR::Node *OMR::LocalCSE::getNode(TR::Node *node)
{
    ...
    if (_simulatedNodesAsArray[node->getGlobalIndex()]) {
        ...
        logprintf(trace(), comp()->log(), "Updating comparison node n%dn to n%dn due to volatile simulation\n",
...
int32_t OMR::LocalCSE::perform()
{
    ...
    logprints(trace(), log, "Starting LocalCommonSubexpressionElimination\n");
    ...
    for (tt = comp()->getStartTree(); tt; tt = exitTreeTop->getNextTreeTop()) {
        ...
        if (doExtraPassForVolatiles()) {
            ...
            logprints(trace(), log, "LocalCSE volatile only pass 1 complete - pass 2 for non-volatiles ONLY\n");
    ...
    int32_t childNum, int32_t *nextLoadIndex, bool *parentCanBeAvailable, int32_t depth)
{
    ...
    logprintf(trace(), log, "Examining node %p\n", node);

    if (!isFirstReferenceToNode(parent, childNum, node, visitCount)) {
        logprints(trace(), log, "\tNot first Reference to Node\n");
    ...
    if (hasAliases && !doneCommoning)
        alreadyKilledAtNonTransparentLoad
            = killExpressionsIfNonTransparentLoad(node, seenAvailableLoadedSymbolReferences, UseDefAliases);
    ...
    if (node->getOpCode().isStore() || hasAliases) {
        ...
        //
        logprintf(trace(), log, "\tnode %p isStore = %d or hasAliases = %d\n", node, node->getOpCode().isStore(),
            hasAliases);
        ...
        if (hasAliases) {
            ...
            if (UseDefAliases.containsAny(tmp, comp())) {
                ...
                if (alreadyKilledAtNonTransparentLoad) // we want to keep the last load of a particular sym ref alive
                                                       // but no earlier loads should surivive
                    seenAvailableLoadedSymbolReferences.set(symRef->getReferenceNumber());
            }
        } else {
...
bool OMR::LocalCSE::canCommonNodeInVolatilePass(TR::Node *node)
{
    return node->getOpCode().hasSymbolReference()
        && (!node->getSymbol()->isTransparent() || node->getSymbol()->isAutoOrParm());
}
...
void OMR::LocalCSE::doCommoningIfAvailable(TR::Node *node, TR::Node *parent, int32_t childNum, bool &doneCommoning)
{
    ...
    if (availableExpression && (availableExpression != node) && shouldCommonNode(parent, node)
        ...
        if (!node->getOpCode().hasSymbolReference()
            ...
            logprintf(trace(), comp()->log(), "Simulating commoning of node n%dn with n%dn - current mode %n\n",
    ...
    TR::SymbolReference *symRef, vcount_t visitCount, bool &doneCopyPropagation)
{
    ...
    if ((!comp()->getOption(TR_MimicInterpreterFrameShape) || !comp()->areSlotsSharedByRefAndNonRef()
        ...
        && (symRef->storeCanBeRemoved()
            || (symRef->getSymbol()->isTransparent() && rhsOfStoreDefNode->getDataType() == TR::Float
...
TR::Node *OMR::LocalCSE::getAvailableExpression(TR::Node *parent, TR::Node *node)
{
    ...
    for (auto it = range.first; it != range.second;) {
        ...
        if (areSyntacticallyEquivalent(other, node, &remove)) {
            logprintf(trace(), log, "node %p is syntactically equivalent to other %p\n", node, other);
            return other;
        }

        if (remove) {
            logprintf(trace(), log, "remove is true, removing entry %p\n", other);
            auto nextIt = it;
            ++nextIt;
            hashTable->erase(it);
            it = nextIt;
...
//
bool OMR::LocalCSE::killExpressionsIfNonTransparentLoad(TR::Node *node,
    TR_BitVector &seenAvailableLoadedSymbolReferences, TR_UseDefAliasSetInterface &UseDefAliases)
{
    ...
    TR_BitVector &seenAvailableLoadedSymbolReferences)
{
    ...
    if (node->canGCandReturn()) {
        logprintf(trace(), log, "Node %p is detected as a GC safe point\n", node);
...
void OMR::LocalCSE::collectAllReplacedNodes(TR::Node *node, TR::Node *replacingNode)
{
    ...
    if (node->getReferenceCount() > 1) {
        ...
        logprintf(trace(), comp()->log(), "Replaced node : %p Replacing node : %p\n", node, replacingNode);
