...
void TR::SwitchAnalyzer::emit(TR_LinkHead<SwitchInfo> *chain, TR_LinkHead<SwitchInfo> *bound,
    TR_LinkHead<SwitchInfo> *earlyUniques)
{
    ...
    switch (_switch->getChild(0)->getDataType()) {
        case TR::Int16:
            rangeLeft = static_cast<CASECONST_TYPE>(
                _signed ? TR::getMinSigned<TR::Int16>() : TR::getMinUnsigned<TR::Int16>());
            rangeRight = static_cast<CASECONST_TYPE>(
                _signed ? TR::getMaxSigned<TR::Int16>() : TR::getMaxUnsigned<TR::Int16>());
        default:
    ...
    if (majorsInEarly > 0) {
        if (majorsInEarly <= MIN_SIZE_FOR_BIN_SEARCH)
            ...
            newBlock = binSearch(earlyUniques->getFirst(), getLastInChain(earlyUniques), majorsInEarly, rangeLeft,
                rangeRight);
    }
...
TR::Block *TR::SwitchAnalyzer::binSearch(SwitchInfo *startNode, SwitchInfo *endNode, int32_t numMajors,
