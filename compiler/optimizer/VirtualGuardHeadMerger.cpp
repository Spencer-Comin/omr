...
int32_t TR_VirtualGuardHeadMerger::perform()
{
    ...
    for (TR::Block *block = optimizer()->getMethodSymbol()->getFirstTreeTop()->getNode()->getBlock(); block;
        ...
        if (isMergeableGuard(guard1)) {
            logprintf(trace(), log, "Found mergeable guard in block_%d\n", block->getNumber());
            ...
            for (TR::Block *nextBlock = block->getNextBlock(); nextBlock; nextBlock = nextBlock->getNextBlock()) {
                if (block->nodeIsRemoved() || nextBlock->nodeIsRemoved()) {
                    logprintf(trace(), log, "block block_%d %p (%d) or nextBlock block_%d %p (%d) has been removed\n",
                ...
                if (guard1->getInlinedSiteIndex() == guard2->getInlinedSiteIndex()) {
                    ...
                    logprintf(trace(), log,
                        "  Guard1 [%p] is the caller of Guard2 [%p] - proceeding with guard merging\n", guard1, guard2);
                } else if ((cold1->getSuccessors().size() == 1) && cold1->hasSuccessor(cold2)) {
                    logprintf(trace(), log,
                        "  Guard1 cold destination block_%d has guard2 cold destination block_%d as its only successor "
                        "- proceeding with guard merging\n",
                ...
                if (guard2Block->nodeIsRemoved()) {
                    logprintf(trace(), log, "guard2Block block_%d %p has been removed after changeBranchDestination\n",
                ...
                if (guard2Tree != guard2Block->getFirstRealTreeTop()) {
                    ...
                    if (!isStopTheWorldGuard(guard2)) {
                        ...
                        if (privargBlock != guard2Block) {
                            logprintf(trace(), log, "  Moving privarg block_%d after block_%d\n",
                    ...
                    logprintf(trace(), log, "  Created new block_%d to hold guard [%p] from block_%d\n",
                ...
                if (insertPoint != guard2Block->getPrevBlock()) {
                    ...
                    logprintf(trace(), log, "  Moving guard2 block block_%d after block_%d\n", guard2Block->getNumber(),
